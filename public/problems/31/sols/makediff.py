    ...
    if os.access(test_dir, os.X_OK):
        for j in range(1,100):
            ...
            if os.access(test_in, os.R_OK):
                print test_in
                (status,output)=commands.getstatusoutput("./%s < %s > %s" % (solution, test_in, "/tmp/tmp_out"))
                if status!=0:
                    error("got error while executing!")
                (status,output)=commands.getstatusoutput("diff %s  %s" % (test_out, "/tmp/tmp_out"))
                if status!=0:
                    error("got differences!")
                    print output
            else:
                break
        else:
